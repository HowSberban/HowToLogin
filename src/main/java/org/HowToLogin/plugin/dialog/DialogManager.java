...
public final class DialogManager {
    ...
    Dialog buildLoginDialog(String locale, Component error, DialogActionCallback onSubmit, DialogActionCallback onCancel) {
        DialogBase base = base(locale, "dialog.login.title", "dialog.login.body", error)
                .inputs(List.of(DialogInput.text("password", text(locale, "dialog.password_label"))
                        ...
                        .build()))
                .build();
        return submitDialog(base, locale, onSubmit, onCancel);
    }

    /** 注册窗口（密码 + 确认输入） */
    Dialog buildRegisterDialog(String locale, Component error, DialogActionCallback onSubmit, DialogActionCallback onCancel) {
    ...
    /** 双因素验证窗口（验证码输入） */
    Dialog build2faDialog(String locale, Component error, DialogActionCallback onSubmit, DialogActionCallback onCancel) {
    ...
    /** 组装提交/取消双按钮并创建 Dialog（confirmation 类型：yes=提交，no=取消，行为由各自回调决定） */
    private Dialog submitDialog(DialogBase base, String locale, DialogActionCallback onSubmit, DialogActionCallback onCancel) {
